...
Python implementation of the solver behind *"Transport-Mechanism
Analysis of Coupled Heat and Mass Transfer in MHD Micropolar Fluid
Flow past a Vertical Plate with Thermal Dispersion, Soret-Dufour
Effects and Chemical Reaction"* (Madhyannapu, Lanka, Sajja, Suresh &
Subbarao), submitted to the International Journal of Heat and Mass
Transfer. All numerical results in the manuscript are produced by
pure-Python/SciPy code.

This code accompanies the manuscript *"Transport-Mechanism Analysis of
...
Chemical Reaction,"* submitted to the International Journal of Heat
and Mass Transfer. The underlying governing equations, similarity
reduction and solver (`model.py`) are unchanged from an earlier
submission of the same study; `transport_budget.py` implements the
transport-budget decomposition, mechanism-isolation comparison and
extended limiting-case reductions added for this submission.
...
| Check | Result |
|---|---|
| Table 3, upper block (M-sweep, 5 rows) | exact match, 6 dp |
| Table 3, lower block (Sr/Du/Kr/Sc one-at-a-time sweeps) | exact match, 6 dp |
| Table 4 (25-point joint Sr,Du grid, both theta'(0) and phi'(0)) | exact match, 6 dp |
| Section 4.4 synergy index (Eq. 15): corner, grid-average | 53.9% / 13.7%, all exact |
| Table 2, lower block (mesh 200/400/800 nodes x eta_inf = 8/10/12/15) | exact match |
| Table 6, upper block (mechanism-isolation comparison) | exact match |
| Table 6, lower block (extended limiting-case reductions, incl. K=0 passive-microrotation limit) | exact match |
...
| Classical Ostrach limit, Pr=0.72 (his own tabulated Pr) | -theta'(0)=0.504631, agrees with a value read directly from Ostrach's 1953 text (Nu=63.6 at Gr_x=1e9) to ~0.23% |
...
| Table 5 (energy- and species-equation transport-budget shares) | exact match; term-by-term residual < 1e-15 at every eta |
| Fig. 8 regularity map: Delta_min vs analytical bound | exact match to machine precision (0.0 discrepancy) across all 25 (Sr,Du) grid points |
| Sensitivity ranking (Section 4.9) | exact match, 6 dp; dominant parameter per output confirmed (K for f''/g', Bi for theta', Sc for phi') |
| Entropy-generation second-law check (Section 4.7) | Ns >= 0 confirmed across the domain and across the full M-sweep, minimum O(1e-9) |
...
- **`shooting_crosscheck.py`** -- an independent verification.
  ...
  adaptive Runge-Kutta/Radau/BDF) combined with a nonlinear root-find
  (`scipy.optimize.fsolve`) on the missing initial slopes `f''(0)`,
  `g'(0)`, `theta(0)`, `phi'(0)`. Run standalone, it reproduces the
  base case and the full M-sweep of Table 3 to six decimal places,
  and independently reproduces both classical-Ostrach validation
  limits (Pr=0.7 and the tighter, exactly benchmarked Pr=1 case).
  The M-sweep is solved by continuation: each M-value is seeded from
  the previously converged root along the path M = 0, 0.25, 0.5, ...,
  2.0, with automatic fallback to a stiffer integrator (Radau, then
  BDF) if the primary method fails to converge at a given step, and a
  clear diagnostic (last converged value, guess carried forward,
  residuals attempted) raised if every fallback fails. This keeps a
  single fixed-guess `fsolve` call from drifting onto the wrong
  solution branch at the larger M values, where the earlier,
  non-continuation version of this script could converge to a
  spurious root. It has not been run against every other table in
  the manuscript (the joint Sr-Du grid, mesh/domain-independence
  study, etc.) -- those rely on the collocation implementation's own
  residual control and mesh/domain independence checks, described in
  the manuscript's Section 3.
...
## Correspondence between code output filenames and the manuscript

The output filenames inside `figures/` (and a few internal function
names in `run_analysis.py`) were set during development and do not
match the manuscript's final numbering one-to-one; several numerical
outputs are consolidated into fewer, two-block tables in the final
manuscript to meet the journal's equivalent-page limit. The mapping
is:
...
| `transport_budget.py` output | **Table 5** (transport-budget shares) |
| mechanism-isolation comparison + extended limiting cases (merged, two blocks) | **Table 6** (case-comparison table) |

The figures `fig2_M`, `fig5_Kr` and `fig7_S` (magnetic-, reaction- and
dispersion-parameter profile plots) are not reproduced as standalone
figures in the IJHMT manuscript, to keep the paper within the
journal's 40-equivalent-page limit; their numerical results remain
fully reported in Table 3 and Table 6 and are unchanged.

No numerical results changed when material was consolidated for the
final manuscript -- only the presentation (which tables/figures are
merged or omitted as standalone display items) was reorganised.
...
requirements.txt for the minimum supported package versions). No GPU or special hardware is
...
parallelism used): `run_analysis.py` typically completes in about 20-25 seconds on a standard laptop CPU; runtime varies with the environment. The independent `shooting_crosscheck.py` typically completes in under 40 seconds for the same reason.
...
All four checks are computed independently by both the collocation
and shooting implementations -- see `ostrach_validation()` and
`rees_pop_validation()` in run_analysis.py, and the corresponding
blocks in shooting_crosscheck.py. The Ostrach checks agree between
the two implementations to at least five decimal places. The Rees &
Pop check is reproduced by the primary collocation implementation to
~0.0001%/0.0013%; the independent shooting implementation gives a
very slightly different but equally strong ~0.0005%/0.0018%, since
the two solvers converge to the ninth-order system by different
routes. The manuscript's Conclusion gives the full derivation and
discussion, and reports the collocation-based figures.
