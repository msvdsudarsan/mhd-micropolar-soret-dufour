...
the joint (Sr,Du) synergy grid of Section 4.4, and the joint (Sr,Du)
...
def table5_M_sweep(P: Params, Mvals):
    print("\nTable 3 (upper block): wall gradients for varying M "
...
def table7_single_sweeps(P: Params):
    ...
    print("\nTable 3 (lower block): wall gradients for varying Sr, Du, Kr, Sc individually "
          f"(M={P.M} fixed)")
...
def table8_srdu_grid(P: Params):
    ...
    print("\nSection 4.4: synergy index S_syn(Sr,Du), Eq. (22), as a % of the "
...
def ostrach_validation(P: Params):
    """Doubly reduced classical Ostrach limit (Section 3.1 / Section 4.10 of the manuscript):
    ...
    print("\nClassical Ostrach limit (Section 3.1 / 4.10): "
...
def table10_K_sensitivity(P: Params):
    """K-sensitivity check: supports the K=0 row of Table 6 (lower block) and the
    ...
    print("\nK-sensitivity check (base case otherwise, M=0.5; supports Table 6, lower block)")
...
def section49_entropy_generation(P: Params):
    ...
    print("\nSection 4.7: entropy generation and Bejan number "
...
def section_regularity_check(P: Params):
    """Section 4.8 verification: confirm Delta_min(eta), computed by direct
    ...
    print("\nSection 4.8: regularity map verification")
...
def table_sensitivity(P: Params):
    """Elasticity ranking: supports the sensitivity discussion in Section 4.9 of the
    manuscript (reported there in prose, not as a standalone table). Relative sensitivity of the four
    ...
    """
    print("\nParameter sensitivity (elasticity) ranking -- supports Section 4.9 discussion")
...
def table_extended_crosscheck(P: Params):
    ...
    paper, including a case close to the regularity boundary of Section 4.8
