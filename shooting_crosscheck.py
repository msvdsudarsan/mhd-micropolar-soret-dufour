...
Table 3, the classical-Ostrach validation limit (Pr=0.7, 0.72 and 1),
...
def solve_shooting(P, eta_inf=10.0, guess=(0.83, -0.10, 1.23, -0.83),
                    ...
                    xtol=1e-12, return_diagnostics=False):
    ...
    resid = np.asarray(info["fvec"], dtype=float)
    converged = (ier == 1) and np.max(np.abs(resid)) < 1e-6
    if not converged:
        if return_diagnostics:
            return None, x, resid, msg
        raise RuntimeError(
            f"shooting failed to converge (ier={ier}, msg='{msg}', "
            f"max|residual|={np.max(np.abs(resid)):.3e})"
        )
    fpp0, gp0, th0, php0 = x
    thp0 = -P["Bi"] * (1 - th0)
    result = (fpp0, -gp0, -thp0, th0, -php0)  # f''(0), -g'(0), -th'(0), th(0), -ph'(0)
    if return_diagnostics:
        return result, x, resid, msg
    return result
...
def solve_shooting_continuation(param_name, targets, base_guess, P_base=None,
                                 eta_inf=10.0, method="RK45", rtol=1e-10,
                                 atol=1e-12, max_step=0.05, xtol=1e-12,
                                 fallback_methods=("Radau", "BDF")):
    """Sweep ``param_name`` over a fine continuation path (``targets``),
    always re-using the previously converged root as the initial guess
    for the next value, so that a single standalone run of this script
    tracks the correct solution branch even far from the base case
    instead of relying on one fixed initial guess for every point.

    Returns a dict {value: (result_tuple, unknowns, max_residual)} for
    every value in ``targets``. Raises RuntimeError with a clear
    diagnostic (last converged value, attempted guess, residual) if a
    step cannot be converged by any of the fallback strategies.
    """
    P_base = base_params() if P_base is None else dict(P_base)
    guess = tuple(base_guess)
    out = {}
    last_ok_value = None
    for value in targets:
        ...
        attempt_log = []
        for meth, rt, at, ms, xt in [
            (method, rtol, atol, max_step, xtol),
            *[(fm, 1e-12, 1e-14, 0.02, 1e-13) for fm in fallback_methods],
        ]:
            result, x, resid, msg = solve_shooting(
                P, eta_inf=eta_inf, guess=guess, method=meth, rtol=rt,
                atol=at, max_step=ms, xtol=xt, return_diagnostics=True,
            )
            attempt_log.append((meth, np.max(np.abs(resid))))
            if result is not None:
                converged_result = (result, x, np.max(np.abs(resid)))
                break
        if converged_result is None:
            attempts_str = ", ".join(f"{m}: max|res|={r:.3e}" for m, r in attempt_log)
            raise RuntimeError(
                f"standalone shooting continuation stalled at "
                f"{param_name}={value}: no method converged.\n"
                f"  Last converged {param_name}: {last_ok_value}\n"
                f"  Guess carried forward: {guess}\n"
                f"  Attempts: {attempts_str}"
            )
        out[value] = converged_result
        guess = converged_result[1]  # continuation: seed the next step
        last_ok_value = value
    return out


if __name__ == "__main__":
    print("Table 3 (M-sweep, upper block), independent shooting-method reproduction:")
    print("Continuation path M = 0 -> 0.25 -> 0.5 -> ... -> 2.0, each step seeded")
    print("from the previously converged root, with automatic fallback to a")
    print("stiffer integrator (Radau/BDF) if the primary method fails to converge.\n")
    print(f"{'M':>5s} {'f`(0)':>10s} {'-g`(0)':>10s} {'-th`(0)':>10s} {'th(0)':>10s} {'-ph`(0)':>10s} {'max|res|':>10s}")
    continuation_path = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]
    reported_M = [0.0, 0.5, 1.0, 1.5, 2.0]
    m_results = solve_shooting_continuation(
        "M", continuation_path, base_guess=(0.83, -0.10, 1.23, -0.83),
        P_base=base_params(),
    )
    for m in reported_M:
        vals, _, max_res = m_results[m]
        print(f"{m:5.2f} " + " ".join(f"{v:10.6f}" for v in vals) + f" {max_res:10.2e}")
